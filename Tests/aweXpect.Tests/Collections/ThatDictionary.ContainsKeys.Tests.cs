		...
		public sealed class WhoseValuesTests
		{
			...
			public async Task WhenKeysExist_ButValuesDoNotMatch_ShouldFail()
			{
				...
				await That(Act).Throws<XunitException>()
					.WithMessage("""
					             Expected that subject
					             has keys [2] whose values should have all items equal to "foo",
					             but not all were
					             """);
			}
			...
			public async Task WhenKeysExist_ButSomeValuesDoNotMatch_ShouldFail()
			{
				...
				await That(Act).Throws<XunitException>()
					.WithMessage("""
					             Expected that subject
					             has keys [1, 2] whose values should have all items equal to "foo",
					             but not all were
					             """);
			}
			...
			public async Task WhenKeysAreMissing_ShouldFail()
			{
				...
				await That(Act).Throws<XunitException>()
					.WithMessage("""
					             Expected that subject
					             has keys [0] whose values should have all items equal to "bar",
					             but it did not have [
					               0
					             ] in [
					               1,
			...
			public async Task WhenOnlySomeKeysAreMissing_ShouldFail()
			{
				...
				await That(Act).Throws<XunitException>()
					.WithMessage("""
					             Expected that subject
					             has keys [1, 0, 3] whose values should have all items equal to "bar",
					             but it did not have [
					               0
					             ] in [
					               1,
			...
			public async Task WhenSubjectIsNull_ShouldFail()
			{
				...
				await That(Act).Throws<XunitException>()
					.WithMessage("""
					             Expected that subject
					             has keys ["foo"] whose values should have all items equal to "",
					             but it was <null>
					             """);
			}
		}
