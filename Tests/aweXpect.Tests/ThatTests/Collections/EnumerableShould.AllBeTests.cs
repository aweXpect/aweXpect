...
public sealed partial class EnumerableShould
{
	public sealed class AllBeTests
	{
		...
		public async Task Item_DoesNotMaterializeEnumerable()
		{
			...
			await That(Act).Should().Throw<XunitException>()
				.WithMessage("""
				             ...
				             but it contained at least 1 other item: [
				               2
				             ]
				             """);
		}
		...
		{
			int constantNumber = 42;
			int[] subject = Factory.GetConstantValueEnumerable(constantNumber, 20).ToArray();

			async Task Act()
				=> await That(subject).Should().AllBe(constantNumber);
		...
		{
			string constantNumber = "foo";
			string[] subject = Factory.GetConstantValueEnumerable(constantNumber, 20).ToArray();
