	...
	public sealed partial class HaveTriggered
	{
		public sealed class WithParameterTests
		{
			...
			public async Task ShouldSupportPositionalParameterFilters(int position, bool expectSuccess)
			{
				...
				async Task Act() =>
					await That(recording).Should()
						.HaveTriggered(nameof(CustomEventWithParametersClass<string, string, string>.CustomEvent))
			...
			public async Task WhenMultipleFiltersAreSpecified_ShouldVerifyAllFilters_ShouldFail()
			{
				...
				async Task Act() =>
					await That(recording).Should()
						.HaveTriggered(nameof(CustomEventWithParametersClass<string, int>.CustomEvent))
						.WithParameter<string>(s => s == "foo")
